...
if (typeof window.Yoke === 'undefined') {
    ...
    window.Yoke = {
        ...
        }, // send gamepad status to Yoke app
        set_bye: function () {
        }, // set pattern for Yoke to send before disconnecting from client
        alert: function (msg) {
            alert(msg);
        } // show prompt on Yoke app
    };
}
...
DPad.prototype.onTouchMove = function (ev) {
    ...
    this.updateStateCallback();
    var currentState = this.stateBuffer.reduce(function (acc, cur) {
        return (acc << 1) + cur;
    }, 0);
    if (currentState != this.oldState) {
        this.oldState = currentState;
        this.updateButtons(currentState);
...
DPad.prototype.updateButtons = function (state) {
    switch (state) {
        case  0:
            this.element.className = 'control dpad';
            break;
        case  1:
            this.element.className = 'control dpad r';
            break;
        case  2:
            this.element.className = 'control dpad d';
            break;
        case  4:
            this.element.className = 'control dpad l';
            break;
        case  8:
            this.element.className = 'control dpad u';
            break;
        case  3:
            this.element.className = 'control dpad dr';
            break;
        case  6:
            this.element.className = 'control dpad dl';
            break;
        case  9:
            this.element.className = 'control dpad ur';
            break;
        case 12:
            this.element.className = 'control dpad ul';
            break;
        default:
            this.element.className = 'control dpad all';
            break;
    }
};
...
function Joypad() {
    ...
    var kernelEvents = this.controls.byNumID.map(function (c) {
        return c.element.id;
    }).join(',');
    if (this.debugLabel != null) {
...
Joypad.prototype.updateDebugLabel = function () {
}; //dummy function
        ...
        switch (id[0]) {
            ...
            case 'm':
                ...
                if (legalLabels.indexOf(id[1]) == -1) {
                    ...
                    if (id.length != 2) {
                        window.Yoke.alert('Please use only one coordinate per motion sensor.');
                    }
...
document.getElementById('menu').childNodes.forEach(function (child) {
    ...
    if (id) {
        ...
        child.addEventListener('click', function () {
            loadPad(id + '.css', pressureSensitive);
        });
    }
});
